...
public:  
  fft3d(unsigned int nx, unsigned int ny, unsigned int nz,
        int sign,Complex *in=NULL, Complex *out=NULL, MPI_Comm myComm=MPI_COMM_WORLD)
    : fftw(2*nx*ny*nz,sign), nx(nx), ny(ny), nz(nz),comm{myComm} {Setup(in,out);}
  
#ifdef __Array_h__
  fft3d(int sign, const Array::array3<Complex>& in,const Array::array3<Complex>& out=Array::NULL3,MPI_Comm myComm=MPI_COMM_WORLD)
    : fftw(2*in.Size(),sign), nx(in.Nx()), ny(in.Ny()), nz(in.Nz()),comm{myComm}
  {Setup(in,out);}
#endif  
  
  fftw_plan Plan(Complex *in, Complex *out) {
#ifdef HAVE_FFTW_MPI
	    return fftw_mpi_plan_dft_3d(nx,ny,nz,(fftw_complex *) in,
	                            (fftw_complex *) out,comm,sign,effort);
#else
...
public:  
  rcfft3d(unsigned int nx, unsigned int ny, unsigned int nz ,
		  Complex *out=NULL, MPI_Comm myComm=MPI_COMM_WORLD)
    : fftw(2*nx*ny*(nz/2+1),-1,nx*ny*nz), nx(nx), ny(ny), nz(nz),comm{myComm}  {Setup(out);}
  
  rcfft3d(unsigned int nx, unsigned int ny, unsigned int nz, double *in,
          Complex *out=NULL, MPI_Comm myComm=MPI_COMM_WORLD) : fftw(nx*ny*realsize(nz,in,out),-1,nx*ny*nz),
                               nx(nx), ny(ny), nz(nz),comm{myComm} {Setup(in,out);}
  
#ifdef __Array_h__
  rcfft3d(unsigned int nz, const Array::array3<Complex>& out, MPI_Comm myComm=MPI_COMM_WORLD)
    : fftw(out.Size(),-1,out.Nx()*out.Ny()*nz),
      nx(out.Nx()), ny(out.Ny()), nz(nz),comm{myComm} {Setup(out);}
  
  rcfft3d(unsigned int nz, const Array::array3<double>& in,
          const Array::array3<Complex>& out=Array::NULL3, MPI_Comm myComm=MPI_COMM_WORLD)
    : fftw(in.Nx()*in.Ny()*realsize(nz,in(),out()),-1,in.Size()),
      nx(in.Nx()), ny(in.Ny()), nz(nz),comm{myComm} {Setup(in,out);}
#endif  
  
  fftw_plan Plan(Complex *in, Complex *out) {
#ifdef HAVE_FFTW_MPI
	    return fftw_mpi_plan_dft_r2c_3d(nx,ny,nz,(double *) in,(fftw_complex *) out,comm,
	                                effort);
#else
...
class crfft3d : public fftw {
  ...
  MPI_Comm comm;
public:  
  crfft3d(unsigned int nx, unsigned int ny, unsigned int nz, Complex *in=NULL,MPI_Comm myComm=MPI_COMM_WORLD)
    : fftw(2*nx*ny*(nz/2+1),1,nx*ny*nz), nx(nx), ny(ny), nz(nz),comm{myComm}
  {Setup(in);} 
  
  crfft3d(unsigned int nx, unsigned int ny, unsigned int nz, Complex *in,
          double *out=NULL,MPI_Comm myComm=MPI_COMM_WORLD) :
    fftw(nx*ny*(realsize(nz,in,out)),1,nx*ny*nz), nx(nx), ny(ny), nz(nz),comm{myComm}
  {Setup(in,out);} 
  
#ifdef __Array_h__
  crfft3d(unsigned int nz, const Array::array3<double>& out,MPI_Comm myComm=MPI_COMM_WORLD)
    : fftw(out.Size(),1,out.Nx()*out.Ny()*nz),
    nx(out.Nx()), ny(out.Ny()), nz(nz),comm{myComm} {Setup(out);}
  
  crfft3d(unsigned int nz, const Array::array3<Complex>& in,MPI_Comm myComm=MPI_COMM_WORLD)
    : fftw(2*in.Size(),1,in.Nx()*in.Ny()*nz),
    nx(in.Nx()), ny(in.Ny()), nz(nz),comm{myComm} {Setup(in);}
  
  crfft3d(unsigned int nz, const Array::array3<Complex>& in,
          const Array::array3<double>& out,MPI_Comm myComm=MPI_COMM_WORLD)
